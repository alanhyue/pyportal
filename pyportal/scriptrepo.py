...
class ScriptRepo:
    ...
    def run_script_by_comment(self, kwd, fpath, comment_exact_match=False):
        return self.run_script_in_commit(
            self.get_commit_by_comment_keyword(kwd, comment_exact_match),
            fpath,
        )
    ...
    def read_script(
        ...
        return (
            self.read_file(fpath, comment_kwd, comment_exact_match, date)
            .getvalue()
            .decode("utf8")
        )

    def read_file(self, fpath, comment_kwd=None, comment_exact_match=False, date=None):
